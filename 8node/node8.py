...
CURRENT_NODE = 'Node 8'
CURRENT_NODE_PORT = 5007  # Port should be an integer
...
def fetch_next_node(source_node, target_node):
    ...
    queue = [(source_node, [source_node])]  # (current node, path to current node)
    visited = set()
    ...
    while queue:
        ...
        if current_node == target_node:
            # Return the next node in the path from source_node
